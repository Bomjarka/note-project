...
DOMAIN_EXISTS := $(shell grep -i "127.0.0.1 note-app.local" /etc/hosts)

install: prepare-env build-clean up prepare-backend

prepare-env: ## Подготовка окружения
			@if [ "$(DOMAIN_EXISTS)" != "127.0.0.1 note-app.local" ]; then echo "127.0.0.1 note-app.local" | sudo tee -a /etc/hosts; fi; \
			cp .env.example .env; \
